...
@pytest.mark.parametrize("status", [429, 999])
def test_a_throttle_status_raises_rate_limited(status: int) -> None:
    """End-to-end: what `raise_if_throttled` raises must be RateLimited.
    A 999 is LinkedIn's own non-standard anti-bot status, which used to
    fall through to a retry loop."""
